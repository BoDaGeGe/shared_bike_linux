...
void cmd_msg_cb(int fd, short events, void* arg)
{
    ...
    {
        mobile_request  mr;
        mr.set_mobile("123456789123123");

        int len = mr.ByteSize();
        memcpy(msg, "FBEB", 4);
        *(u16*)(msg + 4) = 1;//EEVENTID_GET_MOBLIE_CODE_RSP
        *(i32*)(msg + 6) = len;
        mr.SerializeToArray(msg + 10, len);
        printf("msg: %s, &msg: 0x%x, msg length: %d, len: %d\n", msg, &msg, strlen(msg), len);
    ...
    {
        login_request lr;
        lr.set_mobile("123456789123123");
        lr.set_icode(icode);

        int len = lr.ByteSize();
        memcpy(msg, "FBEB", 4);
        *(u16*)(msg + 4) = 3;//EEVENTID_LOGIN_RSP
        *(i32*)(msg + 6) = len;
...
void server_msg_cb(struct bufferevent* bev, void* arg)
{
    ...
    if(strncmp(msg, "FBEB", 4) ==0)
    {
        u16 code = *(u16*)(msg + 4);
        i32 len = *(i32*)(msg + 6);
        if (2 == code)  // EEVENTID_GET_MOBLIE_CODE_RSP
        {
            mobile_response mr;
        ...
        else if (4 == code)  // EEVENTID_LOGIN_RSP
        {
            login_response lr;
