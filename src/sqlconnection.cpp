...
bool MysqlConnection::Init(const char* szHost, int nPort, const char* szUser, const char* szPasswd, const char* szDb)
{
	...
	char cAuto = 1;
	if (mysql_options(mysql_, MYSQL_OPT_RECONNECT, &cAuto) != 0)  // MYSQL_OPT_RECONNECT 没连上的话可以重连
	{
		LOG_ERROR("mysql_options MYSQL_OPT_RECONNECT failed. reason: %s", GetErrInfo());
	}

	if (!mysql_real_connect(mysql_, szHost, szUser, szPasswd, szDb, nPort, NULL, 0))
	{
		LOG_ERROR("connect mysql failed. reason: %s", this->GetErrInfo());
		return false;
	}
